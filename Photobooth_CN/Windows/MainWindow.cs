...
public class MainWindow : Window, IDisposable
{
    ...
    private unsafe void Unopened()
    {
        ...
        ImGui.Text("请创建或编辑一个肖像");
        ImGui.Spacing();

        if (ImGuiComponents.IconButtonWithText(FontAwesomeIcon.Portrait, "肖像列表"))
        {
            UIModule.Instance()->ExecuteMainCommand(92);
        }

        if (ImGuiComponents.IconButtonWithText(FontAwesomeIcon.AddressCard, "冒险者铭牌"))
        {
